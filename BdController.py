...
class BdServer(CommsServer):
   ...
   def makeListener(self, connection):
      ...
      if connection:
         try:
            listener = BdListener(connection, controller=self.controller) # need controller?
...
class BdListener(CommsListener):
   ...
   Listener is started with a receiver (BlueDot object that has been connected) and a defined server object.
   '''
