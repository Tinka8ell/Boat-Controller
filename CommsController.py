# !/usr/bin/python3
# CommsController - base Linked controller
'''
Overview:
Controller  - does what it says on the tin.  Accepts attaching of Servers
            - Allocates listeners to ID's, where 0 is the naviagator, and the rest (>0) are targetting
Servers     - Once accepted, wait for connections.  And creates them into listeners and passes them to the controller.
Listener    - Listen for messages from clients and transaltes them into actions on the controller.
Message     - Speciffic to client, but interpreted as action (press, move, lift, double) and location
'''
...
class CommsController():
   ...
   def addBoat(self, boat):
      ...
      # for now assume 0 to 3 guns, either linear or if 3 and front to "same" then they are port and starboard
      guns = []
   ...
   def navigate(self, connectionId, x, y):
      ### print("CommsController.navigate: connectionId =", connectionId, "(x, y) =", (dp2(x), dp2(y)))
      # default action is to call the boat's naviagation with ID and position
      if connectionId > 0:
         if self.boat:
      ...
      if self.targets == 1:
         ...
         if self.square: # split four ways
            ...
            if radius > 0.3: # only react if outside the middle circle
               # divide in to four quatrants
               # allow a separation of 5% of section
               gun = 3 # starboard
               size = 0.25
               a = angle # map onto quarter
               print("try first: angle =", dp2(angle), "a>.25", angle > 0.25)
               if angle > 0.25: # not 1st quarter
                  print("not first: angle =", dp2(angle), "a<.5", angle < 0.5)
                  gun = 1 # rear starboard
                  if angle > 0.5: # not 2nd quarter
                     print("not second: angle =", dp2(angle), "a<.75", angle < 0.75)
                     a = 1 - angle # map onto other half
                     gun = 0 # back port
                     if angle > 0.75: # not 3rd quarter
                        print("not third")
                        gun = 2 # port
               print("square: a=", dp2(a), "abs(a)=", dp2(abs(a)), "size=", size, "new a=", dp2(a - size * a / abs(a)))
               if abs(a) > size:
                  a += -size * a / abs(a) # map angle on to quarter circle
               d = abs((2 * a / size) - 1) # abs((2 * a - size) / size) # distance from center of range (size)
               print("square: gun=", gun, "size=", size, "a=", dp2(a), "d=", dp2(d))
               if d <= 0.9: # lop off biggest 10% left or right to give separation from other sections
                  f = ((a - (0.05 * size)) / 0.9) / size # express a as fraction of 5% - 95% of the size
                  steps, start, stop, middle = self.guns[gun]
                  s = int(f * steps) # steps using fraction of full steps
            ...
            if radius > 0.3: # only react if outside the middle circle
               # devide in to front quatrants and back semicircle
               # allow a separation of 5% of section
               gun = 2 # starboard
               size = 0.25
               a = angle # map onto quarter
               ...
               d = abs((2 * a / size) - 1) # abs((2 * a - size) / size) # distance from center of range (size)
               print("triangle: gun=", gun, "size=", size, "a=", dp2(a), "d=", dp2(d))
               if d <= 0.9: # lop off biggest 10% left or right to give separation from other sections
                  f = ((a - (0.05 * size)) / 0.9) / size # express a as fraction of 5% - 95% of the size
                  steps, start, stop, middle = self.guns[gun]
                  s = int(f * steps) # steps using fraction of full steps
            ...
            gun = math.ceil(radius * (len(self.guns) + 1)) # 0 to number of guns+1
   ...
   def double(self, connectionId, x, y):
      ...
      if connectionId == 0: # navigate
         if len(self.listeners) < 2:
            self.connected(listener) # prettend we connected as second time
...
class CommsServer(Thread):
   ...
   def startup(self, serverId, controller):
      ### print("CommsServer.startup called", "serverId=", serverId, "controller=", controller)
   ...
   def makeListener(self, connection):
      ...
      listener = None
      if connection:
         listener = CommsListener(connection, controller=self.controller) # do we need controller here?
      return listener
...
class CommsListener(Thread):
   ...
   def __init__(self, connection, controller=None):
      ### print("CommsListener", "connection=", connection, "controller=", controller)
      Thread.__init__(self)
      self.receiver = None
      if connection:
   ...
   def startup(self, connectionId, controller):
      ### print("CommsListener.startup() connectionId=", connectionId, "controller=", controller)
