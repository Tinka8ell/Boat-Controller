...
class DisplayDevice(Canvas):
   ...
   def __init__(self, parent, w, h, bg="blue"):
      Canvas.__init__(self, parent, width=w, height=h, bg=bg, borderwidth=0, highlightthickness=0)
      self.width = w
      self.height = h
      self.parent = parent
      self.makeDisplay()
...
class DisplayMotor(DisplayDevice):
   ...
   def makeDisplay(self):
      ...
      self.forward = self.create_rectangle(0, 0, self.width, self.height // 2, fill="green")
      self.backward = self.create_rectangle(0, self.height // 2, self.width, self.height, fill="red")
      return
...
class DisplayMotors(DisplayDevice):
   ...
   def makeDisplay(self):
      self.create_rectangle(0, 0, self.width, self.height, fill="white", outline="white")
      number = self.number
      w = self.width // (2 * number - 1)
      if number == 1: # only central motor
         motor = DisplayMotor(self.parent, w, self.height, bg="white")
         self.create_window((self.width // 2,  self.height // 2), window=motor)
         self.motors.append(motor)
      elif number == 2: # only left and right motors
         motor = DisplayMotor(self.parent, w, self.height, bg="white")
         self.create_window(((self.width // 2) - w,  self.height // 2), window=motor)
         ...
         motor = DisplayMotor(self.parent, w, self.height, bg="white")
         self.create_window(((self.width // 2) - 2 * w,  self.height // 2), window=motor)
         self.motors.append(motor) # left
         motor = DisplayMotor(self.parent, w, self.height, bg="white")
         self.create_window(((self.width // 2) + 2 * w,  self.height // 2), window=motor)
         self.motors.append(motor) # right
...
class DisplayRudder(DisplayDevice):
   ...
   def makeDisplay(self):
      ...
      self.w = w
      self.l = l
      self.rudder = self.create_line(w, 0, w, l, fill="black", width=w // 10, arrow=LAST)
      return
...
class DisplayGun(DisplayDevice):
   ...
   def makeDisplay(self):
      ...
      self.b = w2
      self.s = s
      self.t1 = self.create_line(w2 - s, self.t, w2 - s, self.b, fill="black", width=w // 10)
      self.t2 = None
      if self.size > 1:
         self.t2 = self.create_line(w2 + s, w2 - l , w2 + s, w2, fill="black", width=w // 10)
      return
   ...
   def adjust(self, value, item=0):
      ...
      coordinates = ((c - s, t), (c - s, b)) # top center to bottom center left by s
      new = []
      for x, y in coordinates:
         v = cangle * (complex(x, y) - center) + center
         new.append(int(v.real))
      ...
      if self.t2:
         coordinates = ((c + s, t), (c + s, b)) # top center to bottom center right by s
         new = []
         for x, y in coordinates:
            v = cangle * (complex(x, y) - center) + center
            new.append(int(v.real))
...
class DisplayGuns(DisplayDevice):
   ...
   def makeDisplay(self):
      ...
      if self.port: # port arangement
         gun = DisplayGun(self.parent, 2 * w, size=self.size, colour=colour, bg="white")
...
class DisplayBoat(BoatListener):
   ...
   def makeDisplay(self):
      ...
      bow = canvas.create_polygon(((c, t), (s, m), (w - s, m)), fill="white", outline="white")
      hull = canvas.create_rectangle(((s, m), (w - s, b)), fill="white", outline="white")
      aft = canvas.create_oval(((s, b - bw2), (w - s, b + bw2)), fill="white", outline="white")
   ...
   def added(self, boat):
      ...
      self.motors = DisplayMotors(canvas, mw, self.motorL, number, bg="white")
      ...
      if self.rear: # have 6 guns, with two side-by-side at rear:
         ...
         self.back = DisplayRearGun(canvas, sizes[self.back], False, size = 2, bg="white")
         canvas.create_window((w - s - width // 2,t3), window=self.back)
      else:
         if self.back:
            self.back = DisplayGun(canvas, sizes[self.back], size = self.back, bg="white")
      ...
      if self.middle and self.front and self.middle == self.front:
         ...
         width = sizes[size]
         height = 2 * width
         self.port = DisplayGuns(canvas, width, height, port=True, size = size, bg="white")
         canvas.create_window((s + width // 2,t1), window=self.port)
         self.starboard = DisplayGuns(canvas, width, height, port=False, size = size, bg="white")
         canvas.create_window((w - s - width // 2,t1), window=self.starboard)
      else:
         if self.middle:
            self.middle = DisplayGun(canvas, sizes[self.middle], size = self.middle, bg="white")
            canvas.create_window((c,t2), window=self.middle)
         if self.front:
            self.front = DisplayGun(canvas, sizes[self.front], size = self.front, bg="white")
...
if __name__ == '__main__':
   ...
   boat = GPIOZeroBoat(left=left, right=right, center=center, rudder=servo, gun=guns)
   displayBoat = DisplayBoat()
   tk = displayBoat.tk
   bdController = BdServer() # None for testing
   test = ControlledBoat(boat=boat, listener=displayBoat, controller=bdController)
   tk.mainloop()
   test.shutdown()
